...
It's all actionable from the row too. Open the document, assign it to a colleague, snooze it until next week or tick it off, and a selection can mix types, so a draft and a task can be handled in one go. There are some AI reads in there as well (summarize, ask about your items, find content gaps), all opt-in and only on a click.
